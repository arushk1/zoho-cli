...
export default class DeskCallsList extends DeskBaseCommand<typeof DeskCallsList> {
  static id = 'desk calls list'
  static summary = 'List Zoho Desk calls'
  ...
  static flags = {
    ...
    status: Flags.string({ description: 'Filter by call status' }),
  }
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      const data = await this.deskGet('/calls', params)
      this.outputSuccess(data.data ?? [], { action: 'desk.calls.list' })
    } catch (error: any) {
