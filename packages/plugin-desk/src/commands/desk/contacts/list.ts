...
export default class DeskContactsList extends DeskBaseCommand<typeof DeskContactsList> {
  static id = 'desk contacts list'
  static summary = 'List Zoho Desk contacts'
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      const data = await this.deskGet('/contacts', params)
      this.outputSuccess(data.data ?? [], { action: 'desk.contacts.list' })
    } catch (error: any) {
