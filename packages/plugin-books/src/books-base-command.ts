...
export type BooksBaseFlags<T extends typeof Command> = Interfaces.InferredFlags<typeof BooksBaseCommand['baseFlags'] & T['flags']>
export type BooksBaseArgs<T extends typeof Command> = Interfaces.InferredArgs<T['args']>

export abstract class BooksBaseCommand<T extends typeof Command> extends Command {
  static baseFlags = {
    ...
    org: Flags.string({
      description: 'Zoho Books organization ID (overrides config)',
      helpGroup: 'GLOBAL',
      env: 'ZOHO_ORG_ID',
    }),
  }

  protected flags!: BooksBaseFlags<T>
  protected args!: BooksBaseArgs<T>
  ...
  public async init(): Promise<void> {
    ...
    const { args, flags } = await this.parse({
      ...
      baseFlags: (super.ctor as typeof BooksBaseCommand).baseFlags,
      args: this.ctor.args,
      strict: this.ctor.strict,
    })
    this.flags = flags as BooksBaseFlags<T>
    this.args = args as BooksBaseArgs<T>
  ...
  protected async resolveOrgId(): Promise<string> {
    ...
    try {
      process.stderr.write('[zoho-cli] Auto-detecting Books organization ID...\n')
      ...
      if (!orgs || orgs.length === 0) {
        this.outputError('NO_ORGS', 'No Zoho Books organizations found for this account')
        this.exit(3)
      }
  ...
  protected get apiClient(): ZohoApiClient {
    if (!this._apiClient) {
      ...
      this._apiClient = new ZohoApiClient({
        ...
        app: 'books',
        version: 'v3',
        baseUrl: `https://www.${domain}/books/v3`,
  ...
  protected async booksGet<R = any>(path: string, params?: Record<string, string>): Promise<R> {
  ...
  protected async booksPost<R = any>(path: string, body?: unknown, params?: Record<string, string>): Promise<R> {
    const orgId = await this.resolveOrgId()
  ...
  protected async booksPut<R = any>(path: string, body?: unknown, params?: Record<string, string>): Promise<R> {
    const orgId = await this.resolveOrgId()
  ...
  protected async booksDelete<R = any>(path: string, params?: Record<string, string>): Promise<R> {
    const orgId = await this.resolveOrgId()
    const { data } = await this.apiClient.delete<R>(path, {
      params: { organization_id: orgId, ...params },
    })
    return data
  }
