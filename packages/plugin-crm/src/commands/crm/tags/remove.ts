...
export default class CrmTagsRemove extends CrmBaseCommand<typeof CrmTagsRemove> {
  static id = 'crm tags remove'
  static summary = 'Remove tags from a record'
  static examples = [
    'zoho crm tags remove Leads 5437280000000328001 --tags "Hot Lead,VIP"',
  ]
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      if (flags['dry-run']) {
        this.outputSuccess(body, { module: args.module, action: 'remove_tags.dry-run' })
        return
      }

      const { data } = await this.apiClient.post(
        `/${args.module}/${args.id}/actions/remove_tags`,
        body,
      )
